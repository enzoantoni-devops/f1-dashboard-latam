...
export function renderHistoricalTable(container, year, tab = 'drivers') {
  ...
  // Fetch de datos - importar dinámicamente
  let promise;
  if (tab === 'drivers') {
    promise = import('../api/client.js').then(module => module.getDriverStandings(year));
  } else {
    promise = import('../api/client.js').then(module => module.getConstructorStandings(year));
  }

  promise.then(standings => {
    renderStandingsTable(container, standings, tab, year);
  }).catch(error => {
    console.error('Error loading historical standings:', error);
    container.innerHTML = `<p>Error cargando datos históricos: ${error.message}</p>`;
  });
}

function renderStandingsTable(container, standings, tab, year) {
  ...
  if (tab === 'drivers') {
    renderDriversTable(container, standings, year);
  } else {
    renderConstructorsTable(container, standings, year);
  }
}

function renderDriversTable(container, drivers, year) {
  const title = `Pilotos (${year})`;
  ...
  // Agregar función de ordenación (solo si no existe ya)
...
function renderConstructorsTable(container, constructors, year) {
  const title = `Constructores (${year})`;
  ...
  // Agregar función de ordenación (misma que drivers)
  window.sortTable = function(columnIndex) {
    const table = container.querySelector('table');
    const tbody = table.querySelector('tbody');
    const rows = Array.from(tbody.querySelectorAll('tr'));
